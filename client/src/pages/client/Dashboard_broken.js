...
const ClientDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const [attendanceResponse, paymentsResponse, workoutResponse] = await Promise.all([
        axios.get(`/api/attendance/client/${user.id}`),
        axios.get(`/api/payments/client/${user.id}`),
        axios.get(`/api/workouts/client/${user.id}`),
      ]);
      ...
      const workoutPlan = workoutResponse.data;
  ...
  const getCurrentMonthAttendance = (attendance) => {
    ...
    return attendance.filter(record => new Date(record.date) >= startOfMonth).length;
  };

  const getCurrentWeekAttendance = (attendance) => {
    const now = new Date();
    const startOfWeek = new Date(now.setDate(now.getDate() - now.getDay()));
    return attendance.filter(record => new Date(record.date) >= startOfWeek).length;
  };
  ...
  const getPaymentStatus = (payments) => {
    const latestPayment = payments[0]; // Assuming sorted by date
    if (!latestPayment) {
      return {
        status: 'pending',
        nextDue: new Date(),
    ...
    const nextDue = new Date(latestPayment.date);
  ...
  return (
    <DashboardContainer>
      ...
      <StatsGrid>
        <StatCard
          title=\"This Month\"
          value={loading ? <Skeleton width={40} /> : dashboardData.attendance.thisMonth}
          change=\"gym visits\"
          icon={<AttendanceIcon />}
        ...
        <StatCard
          ...
          value={loading ? <Skeleton width={40} /> : `${dashboardData.attendance.streak} days`}
          change=\"Keep it up!\"
          changeType=\"positive\"
          icon={<ProgressIcon />}
        />
        <StatCard
          ...
          value={loading ? <Skeleton width={60} /> : `${dashboardData.attendance.percentage}%`}
          change=\"this month\"
          changeType=\"positive\"
          icon={<CalendarIcon />}
        />
        <StatCard
          ...
          value={
            loading ? (
              ...
              <Chip
                label={dashboardData.payments.status.toUpperCase()}
                color={getPaymentStatusColor(dashboardData.payments.status)}
                size=\"small\"
                sx={{ fontWeight: 600 }}
              />
            )
